...
TOP_PT_SF_HISTO_FILENAME = 'pt_distribution_comparisons.root'
TOP_PT_SF_HISTO_NAME = 'sf_plot'
...
#		elif self.lep_type == 0 :
	...
	def __open_top_pT_sf_files__(self) :
		prepend = ''
		if self.on_grid == 'yes' :
			prepend+='./tardir/'
		else :
			prepend+='../other_input_files/'
		self.top_pT_sf_file = ROOT.TFile(prepend+TOP_PT_SF_HISTO_FILENAME)
		self.top_pT_sf_histo = self.top_pT_sf_file.Get(TOP_PT_SF_HISTO_NAME)
	...
	#top pT reweighting, for Powheg and madgraph semileptonic/dileptonic events
	#takes in the MC truth t and tbar vectors
	#returns the reweighting factor, based on the ratio of NLO/LO events (original work)
	def getNewToppT_reweight(self,t_vec,tbar_vec) :
		if (self.generator != 'powheg' and self.generator != 'madgraph') or self.event_type>2 : 
			return 1.0,1.0,1.0
		tpt = t_vec.Pt(); tbarpt = tbar_vec.Pt()
		###############################################
		if tpt>=1000. :
			tpt = 900.
		if tbarpt>=1000. :		#REMOVE THESE LINES WITH BETTER STATS
			tbarpt=900.
		###############################################
		bin1 = self.top_pT_sf_histo.FindFixBin(tpt); bin2 = self.top_pT_sf_histo.FindFixBin(tbarpt)
		f1 = self.top_pT_sf_histo.GetBinContent(bin1); f2 = self.top_pT_sf_histo.GetBinContent(bin2)
		f1_err = self.top_pT_sf_histo.GetBinError(bin1); f2_err = self.top_pT_sf_histo.GetBinError(bin2)
		sf = sqrt(f1*f2)
		sferr = (1./(2*sf))*sqrt(f2*f2*f1_err*f1_err + f1*f1*f2_err*f2_err)
		return sf,sf-sferr,sf+sferr
