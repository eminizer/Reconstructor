...
if __name__ =='__main__':
    ...
    if options.submitCondor or options.dontSubmitCondor:
        ...
        if collection.tarfile:
            ...
            extraLines += 'transfer_input_files = %s, %s, %s, %s\n'%(collection.tarfile,absPath(sys.argv[0]),absPath(commandsFile),collection.bashname())
        #argDict = \
        ...
        argDict = \
                { 'command'     : collection.bashname(),
                  'user'        : commands.getoutput ('whoami'),
                  'arguments'   : '%s %s %s %s $(Process) $(Cluster)' \
                                  % (sys.argv[0].rstrip('runManySections.py'),
                                     commandsFile,
                                     os.environ['SCRAM_ARCH'],
                                     os.environ['CMSSW_BASE'].split('/')[-1]),
                  'numSections' : len (collection),
                  'extraLines'  : extraLines,
                  'condorOutputDir' : condorOutputDir
                  }
        ...
        if options.submitCondor:
            #print 'would have submitted file: %s'%(name)
