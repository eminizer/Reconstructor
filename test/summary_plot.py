...
my_afb = {
	'CMS (35.9 fb^{-1})':{
		...
		'Afb':0.048,
		'statErr':{'up':0.095,'dn':0.087},
		'systErr':{'up':0.020,'dn':0.029},
	},
}
...
class Entry(object) :
	...
	def makeBoxes(self,lmargin,rmargin,top,h) :
		...
		lboxy2 = self._top - 0.01
		lboxy1 = self._top - (2./3.)*h
		rboxy2 = lboxy1
		rboxy1 = lboxy2 - h + 0.025
		if self._is_theory :
			self._lbox = TPaveText(x1+0.0425,lboxy1,xmid,lboxy2,'NDC')
		else :
		...
		#print 'AFB = %s, toterrup = %s, toterrdown = %s'%(str(self._Afb).split('.')[1].rstrip('0'),str(self._totErrUp).split('.')[1].rstrip('0'),str(self._totErrDn).split('.')[1].rstrip('0'))
		ndigits = max(len(str(self._Afb).split('.')[1].rstrip('0')),min(3,len(str(self._totErrUp).split('.')[1].rstrip('0')),len(str(self._totErrDn).split('.')[1].rstrip('0'))))
		nboxstring = ''
		...
		# if it's a theory entry, add the little boxes and lines showing the colors
		if self._is_theory :
			boxx1 = -0.175
			boxy2 = lboxy2 - 0.0185
			boxwidth = 0.02
			boxheight = 0.025
			theory_leg_box = TBox(boxx1,boxy2-boxheight,boxx1+boxwidth,boxy2)
			theory_leg_line = TLine(boxx1,boxy2-0.5*boxheight,boxx1+boxwidth,boxy2-0.5*boxheight)
			theory_leg_line.SetLineWidth(3)
			if self._name=='NNLO QCD (+ NLO EW)' :
				theory_leg_box.SetFillColor(kCyan-9)
				theory_leg_line.SetLineColor(kCyan-6)
			elif self._name=='POWHEGv2 NLO' :
				theory_leg_box.SetFillColor(kMagenta-10)
				theory_leg_line.SetLineColor(kMagenta-9)
			else :
				print 'WARNING: theory entry type %s not recognized in Entry.makeBoxes'%(self._name)
			self._theory_legend_objects.append(theory_leg_box)
			self._theory_legend_objects.append(theory_leg_line)
	...
	def getNbox(self) :
		return self._nbox
	def getTheoryLegendObjects(self) :
		return self._theory_legend_objects
	def getBottom(self) :
		return self._bot
	def getTop(self) :
		return self._top
...
class Plot(object) :
	...
	def __makeAndPlotBoxes__(self) :
		...
		y2 = 1. - self._canvas.GetTopMargin() - self._legend_height - (len(self._categories) + 1)*self._cat_label_height #- self._bg_histo.GetXaxis().GetTickLength() 
		entryboxheight = (y2-y1)/float(self._n_points)
		allboxes = []
		for cat in self._categories :
			caty2 = y2 + (len(self._categories) - self._categories.index(cat))*self._cat_label_height 
			cat.makeEntryBoxes(lm,rm,caty2,entryboxheight)
			allboxes+=cat.getEntryBoxes()
		if self._tevatron_theory != None :
			self._tevatron_theory.makeBoxes(lm,rm,caty2,entryboxheight)
			allboxes+=[self._tevatron_theory.getLbox(),self._tevatron_theory.getRbox(),self._tevatron_theory.getNbox()]+self._tevatron_theory.getTheoryLegendObjects()
		if self._my_measurement != None :
			mymeasy2 = caty2 - self._cat_label_height
			self._my_measurement.makeBoxes(lm,rm,mymeasy2,entryboxheight)
			allboxes+=[self._my_measurement.getLbox(),self._my_measurement.getRbox(),self._my_measurement.getNbox()]
		if self._powheg_theory != None :
			self._powheg_theory.makeBoxes(lm,rm,mymeasy2,entryboxheight)
			allboxes+=[self._powheg_theory.getLbox(),self._powheg_theory.getRbox(),self._powheg_theory.getNbox()]+self._powheg_theory.getTheoryLegendObjects()
		for box in allboxes :
			box.Draw('SAME')
	...
	def __makeAndPlotTheoryLines__(self) :
		...
		self._tevatron_box.SetFillColor(kCyan-9)
		self._powheg_box.SetFillColor(kMagenta-10)
		self._tevatron_line.SetLineColor(kCyan-6)
		self._powheg_line.SetLineColor(kMagenta-9)
	...
	def __makeAndPlotDividers__(self) :
		...
		for cat in self._categories :
			topy = cat.getSectionTop() + 0.5*self._cat_label_height
			...
			newtex = TLatex(-0.172,topy,'p#bar{p} '+cat.getName())
			newtex.SetTextSize(0.035)
			newtex.SetTextAlign(12)
			self._cat_labels.append(newtex)
		#make divider for our measurement
		topy = self._my_measurement.getTop() + 0.5*self._cat_label_height
		boty = self._canvas.GetBottomMargin()+self._bg_histo.GetXaxis().GetTickLength()
		#newline = TLine(self._bg_histo.GetXaxis().GetXmin(),topy,self._bg_histo.GetXaxis().GetXmax(),topy)
		#newline.SetLineWidth(2)
		#self._divider_lines.append(newline)
		newtex = TLatex(-0.172,topy,'pp lepton+jets')
		newtex.SetTextSize(0.035)
		newtex.SetTextAlign(12)
		self._cat_labels.append(newtex)
